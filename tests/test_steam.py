...
@pytest.mark.parametrize(
    "env_val, expected_extra_paths",
    [
        (None, []),
        ("", []),
        (f"/custom/path1{os.pathsep}/custom/path2", [Path("/custom/path1"), Path("/custom/path2")]),
    ]
)
def test_get_compat_tool_dirs_variants(monkeypatch, env_val, expected_extra_paths):
    ...
    if env_val is None:
        ...
        monkeypatch.setenv("STEAM_EXTRA_COMPAT_TOOLS_PATHS", env_val)
    ...
    expected_paths.extend(expected_extra_paths)
