...
# Protontricks will listen to the standard output. Once it's closed,
# the launcher has finished starting up.

exec "$STEAM_RUNTIME_PATH"/run --share-pid --launcher \
"${mount_params[@]}" -- \
--info-fd 1 --bus-name="com.github.Matoking.protontricks.App${STEAM_APPID}_${PROTONTRICKS_SESSION_ID}"
